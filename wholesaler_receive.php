...
$party_id = $_SESSION["party_id"]; // current wholesaler party_id
$bc = new Blockchain($conn);
$msg = "";

// Handle confirm BEFORE reloading list
if ($_SERVER["REQUEST_METHOD"] === "POST" && isset($_POST["confirm"])) {
    $shipment_id  = (int)$_POST["shipment_id"];
    $receive_date = $_POST["receive_date"] ?: date('Y-m-d');

    // Load shipment row (still pending and addressed to this wholesaler)
    $stmt = $conn->prepare("
        SELECT from_party_id, to_party_id, medicine_id, qty
        FROM mfg_wholesaler_shipments
        WHERE id = ? AND to_party_id = ? AND status = 'PENDING'
    ");
    if (!$stmt) {
        die("SQL Error (load single shipment): " . $conn->error);
    }
    ...
    if (!$shipment) {
        ...
        // Check manufacturer stock (safety)
        $chk = $conn->prepare("SELECT qty FROM stock WHERE medicine_id = ? AND party_id = ?");
        if (!$chk) {
            die("SQL Error (check manufacturer stock): " . $conn->error);
        }
        ...
        if ($available < $qty) {
            $msg = "Manufacturer does not have enough stock anymore. (Available: $available)";
        } else {
            // 1) Reduce from manufacturer stock
            $upd = $conn->prepare("
                UPDATE stock SET qty = qty - ? 
                WHERE medicine_id = ? AND party_id = ?
            ");
            if (!$upd) {
                die("SQL Error (reduce manufacturer stock): " . $conn->error);
            }
            $upd->bind_param("iii", $qty, $medicine_id, $from_party);
            $upd->execute();

            // 2) Add to wholesaler stock
            $ins = $conn->prepare("
            ...
            ");
            if (!$ins) {
                die("SQL Error (add wholesaler stock): " . $conn->error);
            }
            $ins->bind_param("iii", $medicine_id, $to_party, $qty);
            $ins->execute();

            // 3) Mark shipment as ACCEPTED
            $updShip = $conn->prepare("
                UPDATE mfg_wholesaler_shipments
            ...
            // 4) Blockchain log
            $bc->addRecord("WHOLESALER_RECEIVED", [
...
// NOW load pending shipments for this wholesaler (after any update)
$sql = "
    SELECT s.id, s.medicine_id, s.qty, s.ship_date, s.status,
           p_from.party_name AS from_name,
           m.generic_name, m.brand_name, m.price
    FROM mfg_wholesaler_shipments s
...
$st = $conn->prepare($sql);
if (!$st) {
    die("SQL Error (load pending shipments): " . $conn->error);
}
...
<h3><i class="bi bi-box-arrow-down"></i> Pending Shipments from Manufacturers</h3>
<p class="text-muted">You only see shipments addressed to you. Confirm to receive stock.</p>
...
<table class="table table-hover table-bordered mb-0">
    ...
    <?php if ($shipments->num_rows == 0): ?>
        <tr><td colspan="5" class="text-center">No pending shipments.</td></tr>
    <?php endif; ?>
